...
for i,(fid,f_df) in enumerate(gp):
    ...
    if i%10000==0:
        ...
        with open("data_propre.log" ,"a+") as f:
            f.write("proprecesed file %d  of %d   run  time  %d \n"%(i,sum_file,run_time))
    proprecess_data['file_id'].append(fid)
    
    proprecess_data['label'].append(f_df['label'].values[0]) #单标签
...
new_df=pd.DataFrame(proprecess_data,columns=['file_id','tid_num','api_text','value_text','label'])
new_df.to_csv("proprecessed_train.csv",index=False)
new_df.head()
